...
use components::unit::{
    Selectable, Unit, UnitAttributes, Velocity, WorkerAnimation, WorkerAnimationState,
};
use resources::{GameState, PlayerResources, ResourceId, ResourceRegistry};
...
use systems::map::{setup_background, spawn_resource_node};
...
use systems::ui::{setup_ui, update_inventory_ui, update_resources_display, update_unit_info}; // Add this import
...
fn main() {
    App::new()
        ...
        .add_systems(
            Startup,
            (setup, setup_ui, setup_window_icon, setup_background),
        )
        .add_systems(
            Update,
            (
